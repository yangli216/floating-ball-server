...
COMMENT ON COLUMN c_ai_device.device_public_key IS '设备 ECDSA P-256 公钥';
...
COMMENT ON COLUMN c_security_rejection_log.id_log IS '安全拒绝日志主键ID';
...
COMMENT ON COLUMN c_security_rejection_log.timestamp_header IS '签名时间戳请求头';
COMMENT ON COLUMN c_security_rejection_log.nonce_header IS '签名随机数请求头';
...
CREATE INDEX idx_security_reject_time ON c_security_rejection_log (insert_time, fg_active);
CREATE INDEX idx_security_reject_type ON c_security_rejection_log (rejection_type, insert_time, fg_active);
CREATE INDEX idx_security_reject_device ON c_security_rejection_log (id_device, insert_time, fg_active);
CREATE INDEX idx_security_reject_path ON c_security_rejection_log (request_path, insert_time, fg_active);
