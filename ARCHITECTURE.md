...
- `modules/emrtemplate` 负责住院病历 HTML 模板解析结果缓存，客户端按 `templateHash` 复用已解析字段；缓存记录保存客户端传入的模板名称和原生 `htmlContent`。管理端支持查询缓存、源码/HTML 预览模板、停用/删除缓存、维护字段 AI 生成提示词。字段提示词覆盖会在客户端解析缓存命中或上传模板解析结果时一并返回，供桌面端生成住院病历预览。
